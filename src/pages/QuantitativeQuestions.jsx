...
const QuantitativeQuestions = ({ onBack }) => {
  ...
  const scales = ['1-5', '1-10', '0-100', 'Percentage'];
  ...
  const fetchQuestions = async () => {
    ...
    setError(null);
    try {
      const data = await quantitativeQuestionsService.getAllQuestions();
      setQuestions(data);
    } catch (err) {
  ...
  const handleAddQuestion = () => {
    ...
    setFormData({ question: '', scale: '1-5' });
    setShowAddForm(true);
  };
  ...
  const handleDeleteQuestion = async (id) => {
    if (window.confirm('Are you sure you want to delete this question?')) {
      try {
        setLoading(true);
        await quantitativeQuestionsService.deleteQuestion(id);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (editingQuestion) {
        ...
        await quantitativeQuestionsService.updateQuestion(editingQuestion.id, formData);
      } else {
        // Add new question
        await quantitativeQuestionsService.createQuestion(formData);
      }
            ...
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center">
                ...
                <h1 className="text-xl font-semibold text-gray-900">
                  {editingQuestion ? 'Edit Quantitative Question' : 'Add New Quantitative Question'}
                </h1>
              </div>
              <div className="flex items-center">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Enter the quantitative question..."
                  required
                />
              </div>
                ...
                {formData.scale === '1-5' && (
                  <div className="mt-2 p-3 bg-blue-50 rounded-md">
                    <p className="text-sm text-blue-700">
                      <strong>5-Point Scale:</strong> 1 = Poor, 2 = Below Average, 3 = Average, 4 = Good, 5 = Excellent
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-900">Quantitative Questions</h1>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl text-red-600 mb-4">
            Quantitative Questions Management
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Create and manage quantitative questions with measurable scales for performance evaluations, KPI tracking, and data-driven assessments.
          </p>
        </div>
            ...
            <div className="divide-y divide-gray-200">
              {questions.length === 0 ? (
                <div className="px-6 py-12 text-center text-gray-500">
                  <p>No questions found. Add your first quantitative question!</p>
                </div>
              ) : (
                      ...
                      {question.scale === '1-5' && (
                        <div className="flex items-center space-x-2 text-sm text-gray-600">
                          ...
                          <div className="flex items-center space-x-1">
                            {[1, 2, 3, 4, 5].map(num => (
                              <span key={num} className="w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center text-xs font-medium">
                                {num}
                              </span>
                            ))}
                          </div>
